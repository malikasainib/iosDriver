...
class HomeViewController: UIViewController {
    ...
    @IBOutlet var cancelrideBTNN: SetButton!
    ...
    func startPulsing() {
       ...
       // self.mStartRideeBTN.applyGradient(colours: [ #colorLiteral(red: 0, green: 1, blue: 0, alpha: 1), #colorLiteral(red: 0.721568644, green: 0.8862745166, blue: 0.5921568871, alpha: 1)])
        mGoBTN.layer.add(animation, forKey: "pulsing")
    ...
    func UpdateLotLng(){
        ...
        if (CLLocationManager.authorizationStatus() == CLAuthorizationStatus.authorizedWhenInUse ||
            ...
            if kRideId != ""{
                ...
                let refs = self.ref?.child("rides").child(name + kRideId).child(kRideId).setValue(dict){ (error, _) in
                    if let error = error {
...
//                                    if Double(cleanedDistanceText)! <= 1.00{
...
//        self.mGoBTN.isHidden = false
        ...
        self.UpdateLotLng()
        self.acceptRejectStatus(confirmStatus: kConfirmationStatus, acceptRejectView: acceptReject.startRideStatus)
    }
}
